...
import { Loader2, Search, PlusCircle, BrainCircuit, Coins, ArrowRightLeft, File as FileIcon, Image as ImageIcon, Music, CheckCircle, HelpCircle } from 'lucide-react';
...
export function AddModelFromProviderDialog({ isOpen, onClose, onAddModels, existingModels }: AddModelFromProviderDialogProps) {
  ...
  const filteredAndSortedModels = useMemo(() => {
      ...
      return allModels
        .filter(model => !existingModelIds.has(model.id) && model.name.toLowerCase().includes(searchTerm.toLowerCase()))
  ...
  const formatPrice = (price: string) => `$${(parseFloat(price) * 1_000_000).toFixed(2)}`;
             ...
             <Table>
                 ...
                 <TableBody>
                     {filteredAndSortedModels.map(model => {
                        ...
                        return (
                            <TableRow key={model.id} className="cursor-pointer" onClick={() => handleSelectionChange(model.id)}>
                                ...
                                <TableCell className="text-xs font-mono">
                                    <div className="flex items-center justify-end gap-1"><ArrowRightLeft className="h-3 w-3 text-green-500" /> <span>{formatPrice(model.pricing.prompt)}</span></div>
                                    <div className="flex items-center justify-end gap-1"><ArrowRightLeft className="h-3 w-3 text-blue-500" /> <span>{formatPrice(model.pricing.completion)}</span></div>
                                </TableCell>
                                 <TableCell className="text-center">
