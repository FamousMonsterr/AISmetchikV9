...
export default function ProPaymentsAdminPage() {
  ...
  const loadOrders = useCallback(async () => {
    if (!user) return;
    setIsLoading(true);
    const result = await getProSubscriptionOrders({
  ...
  const handleApprove = (orderId: string) => {
    ...
    startTransition(async () => {
      const result = await approveProSubscriptionOrder({ adminUserId: user.uid, orderId });
      if (result.success) {
  ...
  const handleReject = (orderId: string) => {
    ...
    startTransition(async () => {
      const result = await rejectProSubscriptionOrder({ adminUserId: user.uid, orderId, reason });
      if (result.success) {
