...
const Ks3Template = ({ contractor, client, sections, contractNumber, contractDate }: Ks3TemplateProps) => (
  <Document>
    {sections.map((section, index) => {
      ...
      return (
        <Page key={`${section.projectName}-${index}`} size="A4" style={styles.page}>
          <Text style={styles.title}>Справка о стоимости выполненных работ (КС-3)</Text>
          <Text style={styles.subtitle}>Договор № {contractNumber || '___'} от {formatContractDate(contractDate)}</Text>
          <View style={styles.section}>
            <Text>Объект: {section.projectName}</Text>
            <Text>Подрядчик: {contractor.fullName || contractor.name}</Text>
