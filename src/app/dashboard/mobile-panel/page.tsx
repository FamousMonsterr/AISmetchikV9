...
export default function MobilePanelPage() {
    ...
    useEffect(() => {
        ...
        const q = query(
            ...
            where('isMainVersion', '==', true),
            orderBy('timestamp', 'desc'),
            limit(1)
        );
        const unsubscribe = onSnapshot(q, (snapshot) => {
            if (!snapshot.empty) {
                const lastProject = { id: snapshot.docs[0].id, ...snapshot.docs[0].data() } as HistoryRequest;
                setLocalCurrentProject(lastProject);
                setGlobalProject(lastProject); 
            }
            setIsLoading(false);
        });
        return () => unsubscribe();
