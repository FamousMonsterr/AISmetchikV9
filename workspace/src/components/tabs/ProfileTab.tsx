...
import { Copy, Bot, User as UserIcon, Send, Save, Loader2, Mail, Briefcase } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
        ...
        <Card>
            ...
            <CardContent className="flex items-center gap-4">
                <Avatar className="h-12 w-12">
                    <AvatarImage src={`https://avatar.vercel.sh/${user.managerData.email}.png`} />
            ...
            <CardFooter>
                 <Button asChild>
                    <a href={`mailto:${user.managerData.email}?subject=Вопрос по EstimateAI`}>
        ...
        <CardContent>
            {user?.telegramChatId ? (
                <Alert variant="default" className="bg-green-50 border-green-200">
                    ...
                    <AlertTitle className="text-green-800">Аккаунт успешно привязан</AlertTitle>
      ...
      {user?.isPartner && (
        <Card>
            <CardHeader>
            <CardTitle>Программа бонусов</CardTitle>
            <CardDescription>Пригласите друга и получите по 100 кредитов каждый!</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
            <p className="text-sm text-muted-foreground">
                Отправьте вашу персональную ссылку другу. После того, как он запустит нашего Telegram-бота по этой ссылке и синхронизирует свой аккаунт, вы оба получите бонус.
            </p>
            <LabelInputContainer>
                <Label>Ваша реферальная ссылка</Label>
                <div className="flex items-center gap-2">
                <Input value={referralLink} readOnly />
                <Button variant="outline" size="icon" onClick={() => handleCopy(referralLink, 'Реферальная ссылка скопирована.')} disabled={!user?.uid}>
                    <Copy className="h-4 w-4" />
                </Button>
                </div>
            </LabelInputContainer>
