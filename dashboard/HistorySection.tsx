...
import { 
    ...
    Link as LinkIcon, FileUp, GitPullRequestCreate, Search, GitBranch
} from "lucide-react";
...
import { onSnapshot, collection, where, orderBy, FirebaseError, query, getDoc, doc } from 'firebase/firestore';
...
export function HistorySection({ 
    ...
    useEffect(() => {
        ...
        const historyQuery = query(
            ...
            where('isMainVersion', '==', true), // Only fetch main versions
...
const HistoryRenderer = (props: HistoryRendererProps) => {
    ...
    const renderContent = () => {
        ...
        if (activeTab === 'active') {
            ...
            emptyDescription = "Начните новый расчет, чтобы увидеть его здесь.";
        } else { // archived
            objectsToRender = archivedObjects;
            ungroupedToRender = ungroupedArchived;
            emptyTitle = "Архив пуст";
            emptyDescription = "Здесь будут находиться архивированные проекты и группы.";
        }
