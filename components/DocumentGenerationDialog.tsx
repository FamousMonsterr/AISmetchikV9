    ...
    const generateFile = async (format: FileFormat): Promise<{ blob: Blob, fileName: string } | null> => {
        ...
        switch (docType) {
            case 'invoice':
                ...
                const invoiceDoc = <InvoiceTemplate invoiceNumber={invoiceNumber} invoiceDate={invoiceDate} seller={contractor} buyer={client} items={[{ name: `Авансовый платеж по договору`, quantity: 1, unit: 'усл', price: advanceAmount }]} />;
            ...
            case 'contract':
                ...
                const contractDoc = (
                    <ContractTemplate
                        ...
                        client={client}
                        objectAddress={project.analysisDetails?.objectName || project.fileName}
